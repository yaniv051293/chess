...
// check if the input is locations at board
bool Chess::isValid() const {
	if (m_input.size() != 4)
		return false;

	char srcCol = m_input[0];
	char srcRow = m_input[1];
	char dstCol = m_input[2];
	char dstRow = m_input[3];

	return (std::isalpha(srcCol) && std::isalpha(dstCol) &&
		std::isdigit(srcRow) && std::isdigit(dstRow) &&
		('A' <= std::toupper(srcCol) && std::toupper(srcCol) <= 'H') &&
		('A' <= std::toupper(dstCol) && std::toupper(dstCol) <= 'H') &&
		('1' <= srcRow && srcRow <= '8') &&
		('1' <= dstRow && dstRow <= '8'));
}
...
void Chess::excute()
{
	...
	//m_boardString[static_cast<size_t>(row) * 8 + col] = '#';
